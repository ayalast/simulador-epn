...
# Comprehensive, Conceptual Pareto Theory Dataset (Physics Heavily Reinforced)
...
parte1_topics = [
    {
        ...
        "stepByStep": [
            {"step": "1", "text": "Determina si el vector velocidad es constante en magnitud y dirección."},
            {"step": "2", "text": "Verifica si el problema pide distancia recorrida (escalar) o desplazamiento (vectorial)."},
            {"step": "3", "text": "Usa la pendiente de $x-t$ para velocidad y el área de $v-t$ para desplazamiento."}
        ],
        "example": {
    ...
    },
    {
        ...
        "variables": ["$F$: fuerza neta ($\\text{N}$)", "$m$: masa inercial ($\\text{kg}$)", "$a$: aceleración ($\\text{m/s}^2$)", "$N$: normal ($\\text{N}$)", "$\\mu_k$: coef. fricción"],
        "sections": [
            {
        ...
        "stepByStep": [
            {"step": "1", "text": "Dibuja el Diagrama de Cuerpo Libre (DCL) aislando únicamente las fuerzas externas sobre el objeto."},
            {"step": "2", "text": "Descompón las fuerzas en ejes paralelo y perpendicular al movimiento."},
            {"step": "3", "text": "Aplica $\\sum F_x = m a$ en el eje del movimiento y $\\sum F_y = 0$ en el eje perpendicular."}
        ],
        "example": {
    ...
    },
    {
        ...
        "variables": ["$p$: momento lineal ($\\text{kg}\\cdot\\text{m/s}$)", "$J$: impulso ($\\text{N}\\cdot\\text{s}$)", "$F$: fuerza media ($\\text{N}$)", "$\\Delta t$: tiempo de contacto ($\\text{s}$)"],
        "sections": [
            {
...
# =========================================================================
# PARTE II: EXAMEN REPORTADO 19 DE AGOSTO (REFORZADO EN FÍSICA)
# =========================================================================
parte2_topics = [
    {
        "id": "p2_fis_01",
    ...
    },
    {
        ...
        "variables": ["$k$: constante elástica ($\\text{N/m}$)", "$x$: deformación/elongación ($\\text{m}$)", "$U_e$: energía elástica ($\\text{J}$)"],
        "sections": [
            {
        ...
        "stepByStep": [
            {"step": "1", "text": "Convierte la deformación a metros: $x = \\frac{\\text{cm}}{100}$."},
            {"step": "2", "text": "Aplica $U_e = \\frac{1}{2}kx^2$."},
            {"step": "3", "text": "Iguala con la energía cinética $\\frac{1}{2}mv^2$ si se solicita la rapidez máxima."}
        ],
        "example": {
    ...
    },
    {
        ...
        "variables": ["$G = 6.674 \\times 10^{-11}\\text{ N}\\cdot\\text{m}^2/\\text{kg}^2$", "$M$: masa del planeta", "$r$: distancia radial al centro"],
        "sections": [
            {
        ...
        "stepByStep": [
            {"step": "1", "text": "Calcula la distancia total al centro: $r = R_{\\text{planeta}} + h$."},
            {"step": "2", "text": "Aplica el factor de escala cuadrático: si $r$ se multiplica por $k$, la fuerza se divide por $k^2$."}
        ],
        "example": {
...
with open('js/study-data-all.js', 'w', encoding='utf-8') as f:
    f.write("/* Banco de Estudio EPN Día 2 — Exact Manus App Replica con Pareto 80/20 Reforzado en Física */\n")
    f.write("window.STUDY_DATA = " + json.dumps(output, indent=2, ensure_ascii=False) + ";\n")

print(f"Generated Rich Theory Dataset with {len(parte1_topics)} Parte I topics, {len(parte2_topics)} Parte II topics and {len(quizzes)} quizzes!")
